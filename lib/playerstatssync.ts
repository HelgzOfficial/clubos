...
export async function syncPlayerStatsFromMatches(): Promise<SyncResult> {
  ...
  // This campaign's competitive fixtures only — friendlies never counted, and
  // now anything before the season opener doesn't either. See lib/season.ts.
  const qualifying = (matches ?? []).filter(countsForSeasonStats);
