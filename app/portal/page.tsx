...
export default function PortalPage() {
  ...
  async function handleBook(e: FormEvent) {
    e.preventDefault();
    if (!player || !bookDate) return;
    setBooking(true);
    setBookError("");
    try {
      // A date, not a time. The player can't see the clinic diary — who else is
      // booked, how long the physio needs, whether the room is free — so the
      // time is the clinician's to set when they confirm. These are placeholder
      // timestamps to keep the day right and the diary sortable; time_set:false
      // is what marks them as not yet real.
      ...
      await createBooking({
        ...
        status: "requested", timeSet: false,
      });
      setShowBook(false);
      setTreatmentType(TREATMENT_TYPE_OPTIONS[0]); setBookDate(todayIso()); setBookNotes("");
                ...
                <li key={b.id} className="flex items-center justify-between gap-2 rounded-lg border border-white/10 px-2.5 py-2 text-xs">
                  <div className="min-w-0">
                    ...
                    <p className="text-neutral-400">
                      ...
                      {/* Until a clinician has set one, there is no time to
                          show — printing the placeholder would read as a real
                          appointment and have players turning up for it. */}
                      {b.time_set ? ` · ${formatTime(b.start_time)}` : " · time to be confirmed"}
                    </p>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-sm max-h-[90vh] overflow-y-auto rounded-card border border-white/10 bg-navy-700 dark:bg-navy-900 p-4 shadow-softDark">
            ...
            <form onSubmit={handleBook} className="space-y-3">
              ...
              <div>
                <label className="mb-1.5 block text-xs font-medium text-neutral-500">Which day?</label>
                <input type="date" value={bookDate} min={todayIso()} onChange={(e) => setBookDate(e.target.value)}
                  className="w-full rounded-xl border border-white/10 bg-navy-600 dark:bg-navy-800 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-club-primary/30" />
                <p className="mt-1.5 text-[11px] text-neutral-500">
                  Pick the day that suits you. The physio sets the time and lets you know once they&apos;ve confirmed it
                  — they can see the whole clinic diary, so they&apos;ll find a slot that works.
                </p>
              </div>
              <div>
